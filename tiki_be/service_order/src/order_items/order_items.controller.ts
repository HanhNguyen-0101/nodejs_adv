...
export class OrderItemsController {
  ...
  async findOne(payload: { id: number }) {
    return await this.orderItemsService.findOne({ orderitemid: +payload.id });
  }

  @MessagePattern('order_items.update')
  async update(payload: { id: number; updateOrderItemDto: UpdateOrderItemDto }) {
    return await this.orderItemsService.update({
      where: { orderitemid: +payload.id },
  ...
  async remove(payload: { id: number }) {
    return await this.orderItemsService.remove({ orderitemid: +payload.id });
  }
}