...
export class CategoriesController {
  ...
  async findOne(payload: { id: number }) {
    return await this.categoriesService.findOne({ categoryid: +payload.id });
  }

  @MessagePattern('categories.update')
  async update(payload: { id: number; updateCategoryDto: UpdateCategoryDto }) {
  ...
  async remove(payload: { id: number }) {
    return await this.categoriesService.remove({ categoryid: +payload.id });
  }
}