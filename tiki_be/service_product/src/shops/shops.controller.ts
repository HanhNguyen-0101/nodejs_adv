...
export class ShopsController {
  ...
  async findOne(payload: { id: number }) {
    return await this.shopsService.findOne({ shopid: +payload.id });
  }

  @MessagePattern('shops.update')
  async update(payload: { id: number; updateShopDto: UpdateShopDto }) {
    return await this.shopsService.update({
      where: { shopid: +payload.id },
  ...
  async remove(payload: { id: number }) {
    return await this.shopsService.remove({ shopid: +payload.id });
  }
}